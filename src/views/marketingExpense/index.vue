  ...
  methods: {
    getExpenseDetail () {
      this.$http.get('/yxexpense/flowYxExpense/queryFlowYxExpenseItemsByMainId', {
        ...
        this.flowYxExpenseItemsList = res.result
      })
    },
    getDetail () {
      this.$http.get('/yxexpense/flowYxExpense/queryById', {
        params: {
          id: this.busiId
        }
      }).then(res => {
    ...
    onSubmit() {
      ...
      if (this.id) {
        ...
        this.formData.flowYxExpenseItemsList = this.flowYxExpenseItemsList
        this.formData.relatedFile = this.relatedFile.join(',')
        this.$http.put('/yxexpense/flowYxExpense/edit', this.formData).then(res => {
          if (res.success) {
            Toast.success('保存成功')
            this.$router.push('/')
          } else {
